...
window.renderElectricalExperience = function renderElectricalExperience(state) {
  ...
  target.innerHTML = `
    ...
    ${view !== 'circuits' ? `<section class="lighting-preview-panel" aria-label="燈光情境預覽">
      ...
      <div class="lighting-type-controls" aria-label="各類燈具統一設定">${['downlight','ceiling_light','pendant'].map(type=>{const c=lighting.controls(electrical,state,type);return `<fieldset class="lighting-type-control"><legend>${escapeHtml(c.label || type)}</legend><label>統一流明 <input type="number" data-light-lumens="${type}" min="100" max="5000" step="50" value="${Number(c.lumens)||0}"><span>lm</span></label><label>統一色溫 <select data-light-kelvin="${type}">${[2700,3000,3500,4000,5000,5700,6500].map(k=>`<option value="${k}" ${Number(c.color_temperature_k)===k?'selected':''}>${k} K</option>`).join('')}</select></label>${type==='downlight'?`<label>照射角度 <input type="range" data-light-beam min="15" max="100" step="5" value="${Math.max(15,Math.min(100,Number(c.beam_angle_deg)||40))}"><output>${Number(c.beam_angle_deg)||40}°</output></label>`:''}<small>套用至此類型所有燈具</small></fieldset>`;}).join('')}</div>
    ...
    <div class="electrical-workspace">
      ...
      <aside class="electrical-inspector electrical-floating-panel ${state.electricalPanelCollapsed?'is-collapsed':''}" id="electricalPreviewPanel">
        ...
        ${view!=='circuits'?`<details class="electrical-light-settings" ${state.electricalLightSettingsOpen?'open':''}><summary>燈光亮度、色溫與角度</summary><label>環境亮度 <input type="range" data-floating-ambient min="0" max="100" step="5" value="${ambientPercent}"><output>${ambientPercent}%</output></label>${['downlight','ceiling_light','pendant'].map(type=>{const c=lighting.controls(electrical,state,type);return `<fieldset><legend>${escapeHtml(c.label||type)}</legend><label>流明 <input type="number" data-floating-lumens="${type}" min="100" max="5000" step="50" value="${Number(c.lumens)||700}"> lm</label><label>色溫 <select data-floating-kelvin="${type}">${[2700,3000,3500,4000,5000,5700,6500].map(k=>`<option value="${k}" ${Number(c.color_temperature_k)===k?'selected':''}>${k} K</option>`).join('')}</select></label>${type==='downlight'?`<label>照射角度 <input type="range" data-floating-beam min="15" max="100" step="5" value="${Number(c.beam_angle_deg)||40}"><output>${Number(c.beam_angle_deg)||40}°</output></label>`:''}</fieldset>`;}).join('')}</details>`:''}
  ...
  const lightSettings=floating.querySelector('.electrical-light-settings');
  lightSettings?.addEventListener('toggle',()=>{state.electricalLightSettingsOpen=lightSettings.open;});
  lightSettings?.querySelector('[data-floating-ambient]')?.addEventListener('input',event=>{state.lightingAmbient=Number(event.target.value);event.target.nextElementSibling.textContent=`${event.target.value}%`;const ambient=target.querySelector('#lightingAmbient');if(ambient)ambient.value=event.target.value;const label=target.querySelector('#lightingAmbientValue');if(label)label.textContent=`${event.target.value}%`;state.electricalLightingController?.(state.lightingAmbient);});
  lightSettings?.querySelectorAll('[data-floating-lumens]').forEach(input=>input.addEventListener('change',()=>{const type=input.dataset.floatingLumens;state.lightingControls??={};state.lightingControls[type]??={};state.lightingControls[type].lumens=Math.max(100,Math.min(5000,Number(input.value)||100));window.renderElectricalExperience(state);}));
  lightSettings?.querySelectorAll('[data-floating-kelvin]').forEach(input=>input.addEventListener('change',()=>{const type=input.dataset.floatingKelvin;state.lightingControls??={};state.lightingControls[type]??={};state.lightingControls[type].color_temperature_k=Number(input.value)||4000;window.renderElectricalExperience(state);}));
  const floatingBeam=lightSettings?.querySelector('[data-floating-beam]');floatingBeam?.addEventListener('input',()=>{floatingBeam.nextElementSibling.textContent=`${floatingBeam.value}°`;});floatingBeam?.addEventListener('change',()=>{state.lightingControls??={};state.lightingControls.downlight??={};state.lightingControls.downlight.beam_angle_deg=Math.max(15,Math.min(100,Number(floatingBeam.value)||40));window.renderElectricalExperience(state);});
  ...
  function selectPoint(id) {
    ...
    details.innerHTML = `<div class="electrical-detail-heading"><span class="electrical-dot" style="background:${escapeHtml(colorOf(point))}"></span><div><small>${escapeHtml(point.id)}</small><h3>${escapeHtml(titleOf(point))}</h3></div></div>
      <dl><div><dt>空間</dt><dd>${escapeHtml(rooms.get(point.space_id)?.name || point.space_id)}</dd></div><div><dt>回路</dt><dd>${escapeHtml(circuit?.name || point.circuit_id)}<small>${escapeHtml(point.circuit_id)}</small></dd></div><div><dt>電壓</dt><dd>${escapeHtml(circuit?.voltage || typeLabels[point.type]?.voltage || '待確認')} V</dd></div><div><dt>安裝高度</dt><dd>${Number.isFinite(height) ? height + ' cm' : '待確認'}</dd></div><div><dt>平面座標</dt><dd>${escapeHtml(point.position[0])}, ${escapeHtml(point.position[1])} cm</dd></div><div><dt>狀態</dt><dd>${escapeHtml(point.status)}</dd></div>${point.control_group_id ? `<div><dt>燈控群組</dt><dd>${escapeHtml(point.control_group_id)}</dd></div>` : ''}${lighting.isFixture(point) ? `<div><dt>模型示意</dt><dd>${escapeHtml(lighting.settings(electrical,point,state).diameter_cm)} cm · ${escapeHtml(lighting.settings(electrical,point,state).lumens)} lm · ${escapeHtml(lighting.settings(electrical,point,state).color_temperature_k)}K${point.type==='downlight'?` · ${escapeHtml(lighting.settings(electrical,point,state).beam_angle_deg)}° 照射角`: ''}<br><small>${lighting.isOn(state,point)?'開燈':'關燈'}</small></dd></div>` : ''}${point.intended_load ? `<div><dt>預定負載</dt><dd>${escapeHtml(point.intended_load)}</dd></div>` : ''}</dl>`;
    target.querySelectorAll('[data-point-id]').forEach(el => el.classList.toggle('is-selected', el.dataset.pointId === id));
    const selected=list.querySelector(`[data-point-id="${CSS.escape(id)}"]`);
    if(selected&&list.clientHeight){const top=selected.getBoundingClientRect().top-list.getBoundingClientRect().top+list.scrollTop;if(top<list.scrollTop)list.scrollTop=top;else if(top+selected.offsetHeight>list.scrollTop+list.clientHeight)list.scrollTop=top+selected.offsetHeight-list.clientHeight;}
  }
  ...
  if(view !== 'circuits') {
    ...
    panel.querySelectorAll('[data-light-lumens]').forEach(input=>input.addEventListener('change',()=>{
      const type=input.dataset.lightLumens;state.lightingControls ??={};state.lightingControls[type] ??={};state.lightingControls[type].lumens=Math.max(100,Math.min(5000,Number(input.value)||100));rerender();
    }));
    panel.querySelectorAll('[data-light-kelvin]').forEach(input=>input.addEventListener('change',()=>{
      const type=input.dataset.lightKelvin;state.lightingControls ??={};state.lightingControls[type] ??={};state.lightingControls[type].color_temperature_k=Number(input.value)||4000;rerender();
    }));
    const beam=panel.querySelector('[data-light-beam]');
    beam?.addEventListener('input',()=>{beam.nextElementSibling.textContent=`${beam.value}°`;});
    beam?.addEventListener('change',()=>{state.lightingControls??={};state.lightingControls.downlight??={};state.lightingControls.downlight.beam_angle_deg=Math.max(15,Math.min(100,Number(beam.value)||40));rerender();});
  ...
  function render3D(walk) {
    ...
    renderer.outputEncoding=THREE.sRGBEncoding;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.2;
    renderer.shadowMap.enabled=!window.matchMedia?.('(pointer:coarse)').matches;renderer.shadowMap.type=THREE.PCFSoftShadowMap;
    renderer.setSize(canvas.clientWidth, canvas.clientHeight);
    renderer.domElement.style.touchAction = 'none';
    canvas.appendChild(renderer.domElement);
    const scene = new THREE.Scene();
    const hemi=new THREE.HemisphereLight(0xffffff,0x777866,1.65);scene.add(hemi);
    const sun = new THREE.DirectionalLight(0xffffff,.65);sun.position.set(floor.bounds.width/2,900,floor.bounds.depth/2-350);sun.target.position.set(floor.bounds.width/2,0,floor.bounds.depth/2);sun.castShadow=true;const shadowSize=Math.max(floor.bounds.width,floor.bounds.depth)/2+90;Object.assign(sun.shadow.camera,{left:-shadowSize,right:shadowSize,top:shadowSize,bottom:-shadowSize});sun.shadow.mapSize.set(1024,1024);sun.shadow.bias=-.0003;scene.add(sun,sun.target);
    ...
    floor.rooms.forEach(room => {
      ...
      const floorMaterial=window.RoomInteriors.roomStyle(state.interiors,room.id)?.surfaces?.floor;
      const finish=state.interiors?.materials?.[floorMaterial]?.finish||(['bedroom','living-dining'].includes(room.category)?'woodgrain':'matte-anti-slip-tile');
      const mesh = new THREE.Mesh(new THREE.ShapeGeometry(shape),window.RoomInteriors.surfaceMaterial(THREE,{color:floorMaterial?window.RoomInteriors.color(state.interiors,floorMaterial):roomColor(room.category),finish,scaleCm:finish==='woodgrain'?22:60}));
      mesh.rotation.x = Math.PI/2;mesh.position.y = -.5;mesh.receiveShadow=true;scene.add(mesh);floorMeshes.push(mesh);
    });
    ...
    markerMeshes.push(...window.ElectricalPointModels.add3D(scene,electrical,floor,visible.filter(point=>!lighting.isFixture(point)),THREE,{showRoutes,pointIds:new Set(visible.map(point=>point.id))}));
    ...
    const cameraKey=walk?'walk':'3d',remembered=state.electricalCameras?.[cameraKey];
    if(walk)camera.position.set(spawnPoint[0],155,spawnPoint[1]);
    if(remembered?.roomFilter===roomFilter){yaw=remembered.yaw;pitch=remembered.pitch;radius=remembered.radius;orbitPhi=remembered.orbitPhi;center.fromArray(remembered.center);if(walk)camera.position.fromArray(remembered.position);}
    teardown.push(()=>{state.electricalCameras??={};state.electricalCameras[cameraKey]={roomFilter,yaw,pitch,radius,orbitPhi,center:center.toArray(),position:camera.position.toArray()};});
    ...
    if (walk && remembered?.roomFilter!==roomFilter) yaw=Math.PI;updateCamera();
