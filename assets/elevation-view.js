...
window.ElevationView = (() => {
  ...
  function draw(floor, room, direction, items=[], electrical=null, showFurniture=true) {
    ...
    const wall=segments.map(s=>`<rect x="${x(s.start)}" y="${y(s.height)}" width="${fmt(s.end-s.start)}" height="${s.height}" fill="#e4e0d6" stroke="#989b91" stroke-width="1"/>`).join('');
    ...
    const furniture=showFurniture?items.filter(item=>item.room_id===room.id).map(item=>{
      ...
      const color=item.kind==='kitchen_wall'?'#f2eee5':item.kind==='kitchen_fridge'?'#e5e3dc':item.kind==='kitchen_hood'||item.kind==='kitchen_cooktop'?'#404341':item.kind==='kitchen_sink'?'#b4c6c6':item.kind==='bed'||item.kind==='sofa'?'#bdb2a6':'#c7a47c';
      ...
      return `<g><title>${escape(item.name)} · ${w} × ${d} × ${h} cm</title><rect x="${x(left)}" y="${y(base+visibleHeight)}" width="${fmt(right-left)}" height="${fmt(visibleHeight)}" fill="${color}" stroke="#716e61" stroke-width="1.5" fill-opacity=".93"/>${right-left>42?`<text x="${x((left+right)/2)}" y="${y(base+visibleHeight/2)}" text-anchor="middle" class="elevation-label">${escape(label)}</text>`:''}</g>`;
    }).join(''):'';
    ...
    const furnitureCount=items.filter(item=>item.room_id===room.id).length;
