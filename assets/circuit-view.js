...
window.ElectricalCircuitView = (() => {
  ...
  function renderOverview(state,floor,canvas,target,selectPoint) {
    ...
    const overlay=`${outlets.map(p=>`<g class="circuit-terminal" data-point-id="${esc(p.id)}" tabindex="0" role="button" aria-label="${esc(p.id)} ${esc(rooms.get(p.space_id)?.name)}" transform="translate(${p.position[0]} ${p.position[1]})"><circle r="13" fill="${esc(data.point_types[p.type]?.render_color||'#4a7b74')}"/><text y="4">${esc(data.point_types[p.type]?.symbol||'S')}</text></g>`).join('')}<g class="circuit-panel" transform="translate(${panel.position[0]} ${panel.position[1]})"><rect x="-17" y="-18" width="34" height="36" rx="5"/><text y="5">盤</text></g>`;
    canvas.innerHTML=window.RoomInteriors.svgBase(floor,{furniture:state.furniture,showFurniture:state.showFurniture,roomColor,labels:true,labelClass:'circuit-room',ariaLabel:'四樓全室插座總覽',children:overlay});;
  ...
  function render(state,floor,canvas,target,selectPoint) {
    ...
    canvas.classList.add('is-2d','is-circuit');
    const overlay=`${routeSvg}${pointSvg}${loadsSvg}<g class="circuit-panel" transform="translate(${panel.position[0]} ${panel.position[1]})"><rect x="-17" y="-18" width="34" height="36" rx="5"/><text y="5">盤</text></g>`;
    canvas.innerHTML=window.RoomInteriors.svgBase(floor,{furniture:state.furniture,showFurniture:state.showFurniture,roomColor,labels:true,labelClass:'circuit-room',ariaLabel:`${circuit.name}示意路徑`,children:overlay});;
