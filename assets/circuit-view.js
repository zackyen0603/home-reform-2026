...
window.ElectricalCircuitView = (() => {
  ...
  function render(state,floor,canvas,target,selectPoint) {
    ...
    const routeSvg=ends.map(e=>{const r=routes.get(e.id);return r?`<polyline class="circuit-route" points="${r.path.map(p=>p.join(',')).join(' ')}" stroke="${color}"/>`:'';}).join('');
    ...
    const distance=Math.max(0,...[...routes.values()].filter(Boolean).map(r=>r.lengthCm))/100;
    ...
    topology.innerHTML=`<div class="circuit-title"><b>${esc(panel.name)} → ${esc(circuit.name)}</b><span>${esc(circuit.voltage)} V · ${esc(circuit.conductor_mm2)} mm²</span></div><p class="micro">${points.length} 個圖面點位 · ${loads.length} 件電器 · 最遠圖面路徑約 ${distance.toFixed(1)} m</p>
      <div class="circuit-tree"><div class="tree-root">配電箱 ${esc(panel.id)}<small>主臥床左側，座標待量測</small></div><div class="tree-branch">分路 ${esc(circuit.id)}<small>${esc(circuit.status)} · 斷路器 ${esc(circuit.breaker_a||'待確認')} A</small></div>${[...grouped].map(([roomId,pp])=>`<details open><summary>${esc(rooms.get(roomId)?.name||roomId)} · ${pp.length} 點</summary>${pp.map(p=>`<button type="button" data-point-id="${esc(p.id)}">${esc(p.id)} · ${esc(data.point_types[p.type]?.label||p.type)}${linked.has(p.id)?`<small>→ ${esc(linked.get(p.id).map(a=>a.name).join('、'))}（圖面推定）</small>`:''}</button>`).join('')}</details>`).join('')}${unlinked.length?`<details open><summary>尚無實際出線點 · ${unlinked.length} 件</summary>${unlinked.map(a=>`<button type="button" data-load-id="${esc(a.id)}">${esc(a.name)}<small>${esc(a.link_status)}</small></button>`).join('')}</details>`:''}${controlGroups.length?`<details><summary>燈控群組 · ${controlGroups.length} 組</summary>${controlGroups.map(g=>`<div class="tree-control">${esc(g.id)} · ${g.point_ids.filter(id=>points.some(p=>p.id===id)).length} 燈</div>`).join('')}</details>`:''}</div>
      <div class="circuit-audit"><h4>走線檢查</h4><p class="micro">${esc(data.route_preview.note)}</p>${checks.length?`<ul>${checks.map(v=>`<li>${esc(v)}</li>`).join('')}</ul>`:'<p>資料未發現可自動判定的缺口，仍須按實際管路與負載覆核。</p>'}<p>電箱及未定點位先依示意座標標示。線徑、壓降、漏電與斷路器匹配均需現場資料才能審核。</p></div>
